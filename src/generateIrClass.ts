 ...
 * one place.
 */
export function generateIrClass(
  endpoint: Endpoint,
  spec: OpenApiSpec,
): IrClassJson {
 ...
 * interfaces.
 */
export function generateIrClassForEndpoints(
  endpoints: Endpoint[],
  spec: OpenApiSpec,
): IrClassJson {
  ...
  for (const ep of endpoints) {
    for (const typeName of collectNeededTypes(
  ...
  for (const ep of endpoints) {
    ...
    const inlineObjects = collectInlineObjectSchemas(
      ep.responseSchema,
      "response",
    );
    ...
    for (const inlineObj of inlineObjects) {
      ...
      const classDef = convertSchemaToIrClass(
        name,
        inlineObj.schema,
        interfaceMap,
      );
...
function generateUniqueName(
  schema: SchemaLike,
  existingClasses: IrClassDefinition[],
  path?: string,
): string {
  ...
  if (schema.properties) {
    ...
    if (propNames.length > 0) {
      const baseName =
        propNames[0].charAt(0).toUpperCase() + propNames[0].slice(1);
      let name = baseName;
      let counter = 1;
...
/** Whether a schema is an inline object with named properties. */
function isObjectSchema(schema: unknown): schema is SchemaLike {
  return (
    !!schema &&
    typeof schema === "object" &&
    "type" in schema &&
    (schema as SchemaLike).type === "object" &&
    !!(schema as SchemaLike).properties
  );
}
 ...
 *
 * The interface is named from the function name (via the shared helper) so the
 * function generator can reference it directly. The payload is the object inside
 * a `{ data: <object> }` envelope when present, or the flat object itself for
 * endpoints with no envelope (e.g. `GET /`).
 */
function generateResponseTypeInterface(
  endpoint: Endpoint,
): IrClassDefinition | null {
  ...
  if (!respSchema || typeof respSchema !== "object" || !respSchema.properties) {
    return null;
  }

  const dataProp = respSchema.properties.data as SchemaLike | undefined;
  let payload: SchemaLike | undefined;
  if (dataProp && isObjectSchema(dataProp)) {
    // Payload is the object inside a `{ data: <object> }` envelope.
    payload = dataProp;
  } else if (!dataProp && isObjectSchema(respSchema)) {
    // Payload is the flat object itself (e.g. GET /).
    payload = respSchema;
  } else {
    return null;
  }

  // Derive the interface name from the function name so it matches what the
  // function generator references for inline-object return types.
  const interfaceName = getResponseTypeInterfaceName(endpoint);

  // Build fields from the payload's properties.
  ...
  if ("properties" in payload && payload.properties) {
    for (const [fieldName, fieldSchema] of Object.entries(payload.properties)) {
      ...
      if ("required" in payload && Array.isArray(payload.required)) {
        isRequired = payload.required.includes(fieldName);
      }

      const comment =
        (fieldSchema as { description?: string }).description || undefined;
  ...
  if (schema.type) {
    switch (schema.type) {
      ...
      case "number":
        return "number";
      case "integer":
        return "number"; // integer is a subset of number
      case "boolean":
        return "boolean";
      case "array":
...
function collectNeededTypes(
  ...
  function extractRefs(obj: unknown, depth = 0) {
    ...
    if (
      "items" in objAsRecord &&
      objAsRecord.items &&
      typeof objAsRecord.items === "object"
    ) {
    ...
    if (
      "additionalProperties" in objAsRecord &&
      objAsRecord.additionalProperties &&
      typeof objAsRecord.additionalProperties === "object"
    ) {
...
function collectInlineObjectSchemas(
  schema: unknown,
  basePath = "root",
): InlineObject[] {
